...
const MyTasks = () => {
  ...
  return (
    <div>
                  ...
                  !IsLoading && currentTasks.length > 0 && currentTasks?.map((item, i) => (
                    <tr key={item.id} className="hover:bg-gray-200">
                      ...
                      <td className="py-4 border-b border-grey-light">
                        ...
                        {user.role === 'manager' || !item.status.disabled ? <button onClick={() => prepareToUpdateOrShow(item.id, 'update')} className="ml-4 mr-3"><FaEdit className="hover:text-second text-primary_text" fontSize={22}/></button> : ''}
                      </td>
                    </tr>
                  ))
              }
      ...
      {ShowModalUpdate && <TaskModal user={user} seterrorMessage={seterrorMessage} closeModal={setShowModalUpdate} header='Update Status' action='Update Status' task={TaskInfo} baseUrl={baseUrl} Tasks={Tasks} setTasks={setTasks} setFilteredTasks={setFilteredTasks}/>}
      {ShowModalShow && <TaskModal user={user} closeModal={setShowModalShow} header='Show Task' action="Show My Task" task={TaskInfo} baseUrl={baseUrl} />}
    </div>
  )
}
